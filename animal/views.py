...
def add_food_products(request):
    if request.method=="POST":
        ...
        if food_form.is_valid():
            cp=FoodProducts(user=request.user,food_name=food_form.cleaned_data['food_name'],
...
def search_place(request):
    if request.method=="GET":
        ...
        print(data)
        date2=str(data)
        cr = Doctor.objects.filter(place=date2)
        return render(request,"view_doctors_place.html",{'cr':cr})
    else:
        return render(request,"view_doctors_place.html")
        print("problem")
